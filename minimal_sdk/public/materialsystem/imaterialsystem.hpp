...
#include "bitmap/imageformat.hpp"
#include "vtf.hpp"
...
using MaterialBufferReleaseFunc_t = void ( * )();
using MaterialBufferRestoreFunc_t =
    void ( * )( int nChangeFlags );  // see RestoreChangeFlags_t
...
constexpr const char* MATERIAL_SYSTEM_INTERFACE_VERSION = "VMaterialSystem080";
...
public:
    ...
    virtual CreateInterfaceFn Init(
        const char* pShaderAPIDLL, IMaterialProxyFactory* pMaterialProxyFactory,
    ...
    virtual void GetDisplayAdapterInfo( int adapter,
                                        MaterialAdapterInfo_t& info ) const = 0;
    ...
    virtual void GetModeInfo( int adapter, int mode,
                              MaterialVideoMode_t& info ) const = 0;
    ...
    // Sets the mode...
    virtual bool SetMode( void* hwnd,
                          const MaterialSystem_Config_t& config ) = 0;
    ...
    virtual void GetDXLevelDefaults( unsigned int& max_dxlevel,
                                     unsigned int& recommended_dxlevel ) = 0;
    ...
    virtual void UncacheUnusedMaterials(
        bool bRecomputeStateSnapshots = false ) = 0;
    ...
    // nonblocking)
    virtual bool IsMaterialLoaded( const char* pMaterialName ) = 0;
    ...
    virtual ITexture* FindTexture( const char* pTextureName,
    ...
    virtual void AddTextureAlias( const char* pAlias,
                                  const char* pRealName ) = 0;
    ...
    virtual IMaterial* FindMaterialEx(
        const char* pMaterialName, const char* pTextureGroupName, int nContext,
        bool complain = true, const char* pComplainPrefix = nullptr ) = 0;
    ...
    virtual void AsyncFindTexture( const char* pFilename,
                                   const char* pTextureGroupName,
                                   IAsyncTextureOperationReceiver* pRecipient,
                                   void* pExtraArgs, bool bComplain = true,
                                   int nAdditionalCreationFlags = 0 ) = 0;
    ...
    virtual ITexture* CreateNamedTextureFromBitsEx(
        ...
        int mips, ImageFormat fmt, int srcBufferSize, uint8_t* srcBits,
        int nFlags ) = 0;
};