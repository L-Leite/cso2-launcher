...
constexpr const char* VGUI_IVGUI_INTERFACE_VERSION = "VGUI_ivgui008";
...
public:
    ...
    // makes panel receive a 'Tick' message every frame (~50ms, depending on
    // sleep times/framerate) panel is automatically removed from tick signal
    // list when it's deleted
    virtual void AddTickSignal( VPANEL panel,
                                int intervalMilliseconds = 0 ) = 0;
    ...
    virtual void AssociatePanelWithContext( HContext context,
                                            VPANEL pRoot ) = 0;
    ...
    virtual void AddTickSignalToHead( VPANEL panel,
                                      int intervalMilliseconds = 0 ) = 0;
};
};  // namespace vgui
