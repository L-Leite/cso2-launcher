...
#include "inputsystem/InputEnums.hpp"
...
#define JOYSTICK_BUTTON_INTERNAL( _joystick, _button )                    \
    ( JOYSTICK_FIRST_BUTTON + ( (_joystick)*JOYSTICK_MAX_BUTTON_COUNT ) + \
      ( _button ) )
...
#define JOYSTICK_AXIS_BUTTON_INTERNAL( _joystick, _button ) \
    ( JOYSTICK_FIRST_AXIS_BUTTON +                          \
      ( (_joystick)*JOYSTICK_AXIS_BUTTON_COUNT ) + ( _button ) )
...
enum ButtonCode_t
{
    ...
    NOVINT_FIRST =
        JOYSTICK_LAST +
        2,  // plus 1 missing key. +1 seems to cause issues on the first button.
