                  ...
                  char const* pchFileName, char const* pchSearchPath )
{
    ...
    if ( fh != FILESYSTEM_INVALID_HANDLE )
    {
        ...
        {
            g_pFullFileSystem->Write( list[i].String(),
                                      Q_strlen( list[i].String() ), fh );
                  ...
                  char const* pchSearchPath )
{
    ...
    {
        int nTokenSize =
            buffer.ParseToken( &breakSet, szToken, sizeof( szToken ) );
        if ( nTokenSize <= 0 )
        {
            break;
        }
...
void CResListGenerator::CollateFiles( char const* pchResListFilename )
{
    ...
    {
        char fn[MAX_PATH];
        Q_snprintf( fn, sizeof( fn ), "%s\\%s\\%s\\%s",
                    m_sFullGamePath.String(), m_sWorkingDir.String(),
...
void CResListGenerator::Init( char const* pchBaseDir, char const* pchGameDir )
{
    ...
    // Because we have to call this inside the first Apps "PreInit", we need
    // only Init on the very first call
    if ( m_bInitialized )
    {
        return;
    }
                                         ...
                                         char const* pchCommandFile )
{
    ...
    char fullpath[512];
    Q_snprintf( fullpath, sizeof( fullpath ), "%s/%s", pchGameDir,
                pchCommandFile );
    ...
    m_nCurrentWorkItem =
        clamp( m_nCurrentWorkItem, 0, m_WorkItems.Count() - 1 );
