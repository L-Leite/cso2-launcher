...
bool FileSystem_GetExecutableDir( char *exedir, int exeDirLen )
{
    ...
    V_snprintf( exedir, exeDirLen, "%s",
                GetCSO2PreCommandLineParser()->GetAnsiBinDirectory() );
...
void LaunchVConfig()
{
    ...
    Q_strncat( vconfigExe, "vconfig.exe", sizeof( vconfigExe ),
               COPY_ALL_CHARACTERS );
...
FSReturnCode_t SetupFileSystemError( bool bRunVConfig,
                                     FSReturnCode_t retVal,
                                     const char *pMsg,
                                     ... )
{
    va_list marker;
                                            ...
                                            bool bLowViolence )
{
         ...
         Q_stricmp( pPathID, "game" ) == 0 )
    {
        if ( CommandLine()->FindParm( "-tempcontent" ) != 0 )
        {
            ...
            Q_snprintf( szPath, sizeof( szPath ), "%s_tempcontent",
                        fullLocationPath );
            initInfo.m_pFileSystem->AddSearchPath( szPath, pPathID,
                                                   PATH_ADD_TO_TAIL );
        }
    }
         ...
         V_strstr( fullLocationPath, "_english" ) != NULL )
    {
        ...
        Q_snprintf( szLangString, sizeof( szLangString ), "_%s",
...
FSReturnCode_t FileSystem_LoadSearchPaths( CFSSearchPathsInit &initInfo )
{
    ...
    FSReturnCode_t retVal = LoadGameInfoFile(
        initInfo.m_pDirectoryName, pMainFile, pFileSystemInfo, pSearchPaths );
    if ( retVal != FS_OK )
        return retVal;

    // All paths except those marked with |gameinfo_path| are relative to the
    // base dir.
...
#define BASESOURCEPATHS_TOKEN "|all_source_engine_paths|"
    ...
    if ( pszExtraSearchPath )
    {
        ...
        FOR_EACH_VEC( vecPaths, idxExtraPath )
        {
            ...
            if ( !V_RemoveDotSlashes( szAbsSearchPath ) )
                Error(
                    "Bad -insert_search_path - Can't resolve pathname for '%s'",
                    szAbsSearchPath );
            V_StripTrailingSlash( szAbsSearchPath );
            FileSystem_AddLoadedSearchPath( initInfo, "GAME", szAbsSearchPath,
                                            false );
            FileSystem_AddLoadedSearchPath( initInfo, "MOD", szAbsSearchPath,
                                            false );
        }
    }
          ...
          pCur = pCur->GetNextValue() )
    {
        ...
        {
            // This is a special identifier that tells it to add the specified
            // path for all source engine versions equal to or prior to this
            // version. So in Orange Box, if they specified:
            //		|all_source_engine_paths|hl2
            // it would add the ep2\hl2 folder and the base (ep1-era) hl2
            // folder.
            //
            // We need a special identifier in the gameinfo.txt here because the
            // base hl2 folder exists in different places. In the case of a game
            // or a Steam-launched dedicated server, all the necessary prior
            // engine content is mapped in with the Steam depots, so we can just
            // use the path as-is.
        ...
        // Don't bother doing any wildcard expansion unless it has wildcards.
        // This avoids the weird thing with xxx_dir.vpk files being referred to
        // simply as xxx.vpk.
            ...
            if ( pszFoundShortName )
            {
                do
                {
                    ...
                    if ( pszFoundShortName[0] != '.' &&
                         ( initInfo.m_pFileSystem->FindIsDirectory(
                               findHandle ) ||
                    ...
                    pszFoundShortName =
                        initInfo.m_pFileSystem->FindNext( findHandle );
        ...
        FOR_EACH_VEC( vecFullLocationPaths, idxLocation )
        {
            FOR_EACH_VEC( vecPathIDs, idxPathID )
            {
                FileSystem_AddLoadedSearchPath(
                    initInfo, vecPathIDs[idxPathID],
                    vecFullLocationPaths[idxLocation], bLowViolence );
            }
        }
    }
    ...
    // Also, mark specific path IDs as "by request only". That way, we won't
    // waste time searching in them when people forget to specify a search path.
...
static FSReturnCode_t TryLocateGameInfoFile( char *pOutDir,
                                             int outDirLen,
                                             bool bBubbleDir )
{
    ...
    do
    {
        ...
        if ( IsX360() &&
             DoesFileExistIn( pOutDir, GAMEINFO_FILENAME_ALTERNATE ) )
        {
            return FS_OK;
        }
                                   ...
                                   int outDirLen )
{
    ...
    if ( fsInfo.m_bOnlyUseDirectoryName )
    {
        ...
        bool bExists =
            DoesFileExistIn( fsInfo.m_pDirectoryName, GAMEINFO_FILENAME );
        if ( IsX360() && !bExists )
        {
            bExists = DoesFileExistIn( fsInfo.m_pDirectoryName,
                                       GAMEINFO_FILENAME_ALTERNATE );
        }
        if ( !bExists )
        {
    ...
    if ( pProject )
    {
        ...
        if ( IsX360() &&
             DoesFileExistIn( pProject, GAMEINFO_FILENAME_ALTERNATE ) )
        {
            Q_MakeAbsolutePath( pOutDir, outDirLen, pProject );
            return FS_OK;
        }
        ...
        {
            // fsInfo.m_bNoGameInfo is set by the Steam dedicated server, before
            // it knows which mod to use. Steam dedicated server doesn't need a
            // gameinfo.txt, because we'll ask which mod to use, even if -game
        ...
        {
            // They either specified vproject on the command line or it's in
            // their registry. Either way, we don't want to continue if they've
            // specified it but it's not valid.
            goto ShowError;
        }
    }
    ...
    // Ask the application if it can provide us with a game info directory
    {
        ...
        SuggestGameInfoDirFn_t pfnSuggestGameInfoDirFn =
            GetSuggestGameInfoDirFn();
        if ( pfnSuggestGameInfoDirFn &&
             ( *pfnSuggestGameInfoDirFn )( &fsInfo, pOutDir, outDirLen,
                                           &bBubbleDir ) &&
...
bool DoesPathExistAlready( const char *pPathEnvVar, const char *pTestPath )
{
    ...
    Q_strncpy( correctedPathEnvVar, pPathEnvVar,
               sizeof( correctedPathEnvVar ) );
    ...
    while ( 1 )
    {
        ...
        // Ok, we found pTestPath in the path, but it's only valid if it's
        // followed by an optional slash and a semicolon.
...
FSReturnCode_t GetSteamCfgPath( char *steamCfgPath, int steamCfgPathLen )
{
    ...
    Q_strncat( steamCfgPath, "steam.cfg", steamCfgPathLen,
               COPY_ALL_CHARACTERS );

    return FS_OK;
}

void SetSteamAppUser( KeyValues *pSteamInfo,
                      const char *steamInstallPath,
                      CSteamEnvVars &steamEnvVars )
{
    // Always inherit the Steam user if it's already set, since it probably
    // means we (or the the app that launched us) were launched from Steam.
    ...
    else
    {
        ...
        Q_strncat( fullFilename, "config\\SteamAppData.vdf",
                   sizeof( fullFilename ), COPY_ALL_CHARACTERS );
...
void SetSteamUserPassphrase( KeyValues *pSteamInfo,
                             CSteamEnvVars &steamEnvVars )
{
    // Always inherit the passphrase if it's already set, since it probably
    // means we (or the the app that launched us) were launched from Steam.
                                                ...
                                                bool &bSteam )
{
    ...
    // the filesystem_stdio or filesystem_steam is in this same directory with
    // us.
    char executablePath[MAX_PATH];
    if ( !FileSystem_GetExecutableDir( executablePath,
                                       sizeof( executablePath ) ) )
...
// Use filsystem_steam if it exists?
    ...
    {
        Q_snprintf( pFileSystemDLL, nMaxLen,
                    "%s%cfilesystem_steam" DLL_EXT_STRING, executablePath,
...
FSReturnCode_t FileSystem_SetupSteamEnvironment( CFSSteamSetupInfo &fsInfo )
{
    ...
    if ( ret != FS_OK )
        ...
        // This is so that processes spawned by this application will have the
        // same VPROJECT
#ifdef WIN32
...
FSReturnCode_t FileSystem_MountContent( CFSMountContentInfo &mountContentInfo )
{
    ...
    if ( mountContentInfo.m_pFileSystem->IsSteam() )
    {
        ...
        //		{
        //			// !FIXME! Here we need to mount the tools content (VPK's)
        //in some way...?
        //		}
        //
        ...
        //			return SetupFileSystemError( true, FS_UNABLE_TO_INIT, "Unable
        //to mount Steam content in the file system" );
    }
