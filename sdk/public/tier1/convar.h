...
{
public:
    virtual int CommandCompletionCallback(
        const char* pPartial, CUtlVector<CUtlString>& commands ) = 0;
};
...
#ifdef __MINGW32__
    ~ConCommandBase() { _ConCommandBase_destructor(); }
    virtual void _ConCommandBase_destructor();
#else
...
#ifdef __MINGW32__
    ~ConCommand() { _ConCommand_destructor(); }
    virtual void _ConCommand_destructor();
#else
...
#ifdef __MINGW32__
    ~ConVar() { _ConVar_destructor(); }
    virtual void _ConVar_destructor();
#else
    virtual ~ConVar( void );
#endif
...
#define CON_COMMAND_MEMBER_F( _thisclass, name, _funcname, description, \
                              flags )                                   \
    void _funcname( const CCommand& args );                             \
