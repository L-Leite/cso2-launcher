...
void* operator new[]( size_t nSize, int blah, const char* pFileName,
                      int nLine );
#endif
...
#define malloc( s ) g_pMemAlloc->AllocDbg( s, __FILE__, __LINE__ )
#define realloc( p, s ) g_pMemAlloc->ReallocDbg( p, s, __FILE__, __LINE__ )
#define _aligned_malloc( s, a ) \
    MemAlloc_AllocAligned( s, a, __FILE__, __LINE__ )
                              ...
                              unsigned nLine )
{
    ...
    if ( ( pMemory = (char*)g_pMemAlloc->AllocDbg( len, pFileName, nLine ) ) !=
         NULL )
    {
...
inline wchar_t* MemAlloc_WcStrDup( const wchar_t* pString,
                                   const char* pFileName, unsigned nLine )
{
    wchar_t* pMemory;
    ...
    if ( ( pMemory = (wchar_t*)g_pMemAlloc->AllocDbg(
               len * sizeof( wchar_t ), pFileName, nLine ) ) != NULL )
    {
...
inline wchar_t* MemAlloc_WcStrDup( const wchar_t* pString )
{
    ...
    size_t len = ( wcslen( pString ) + 1 );
    if ( ( pMemory = (wchar_t*)g_pMemAlloc->Alloc(
               len * sizeof( wchar_t ) ) ) != NULL )
    {
        return wcscpy( pMemory, pString );
    }
