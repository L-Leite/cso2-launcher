...
public:
    ...
    virtual void *AllocDbg( size_t nSize, const char *pFileName, int nLine ) = 0;

	virtual void *Realloc( void *pMem, size_t nSize ) = 0;
    virtual void *ReallocDbg( void *pMem, size_t nSize, const char *pFileName, int nLine ) = 0;

	virtual void Free( void *pMem ) = 0;
    virtual void FreeDbg( void *pMem, const char *pFileName, int nLine ) = 0;

    virtual void *Expand_NoLongerSupported( void *pMem, size_t nSize ) = 0;
    virtual void *Expand_NoLongerSupportedDbg( void *pMem, size_t nSize, const char *pFileName, int nLine ) = 0;
...
inline void *MemAlloc_Alloc( size_t nSize, const char *pFileName, int nLine )
{ 
	return g_pMemAlloc->AllocDbg( nSize, pFileName, nLine );
}
#endif
...
{
    g_pMemAlloc->FreeDbg( ptr, pFileName, nLine );
}
...
inline void *MemAlloc_AllocAligned( size_t size, size_t align, const char *pszFile, int nLine )
{
    ...
    if ( ( pAlloc = (unsigned char*)g_pMemAlloc->AllocDbg(
...
inline void MemAlloc_FreeAligned( void *pMemBlock, const char *pFileName, int nLine )
{
	...
	// pAlloc is the pointer to the start of memory block
	pAlloc = *( (void **)pAlloc );
    g_pMemAlloc->FreeDbg( pAlloc, pFileName, nLine );
}
...
#if defined(_WIN32) && !defined(COMPILER_MINGW) && ( defined(_DEBUG) || defined(USE_MEM_DEBUG) )
