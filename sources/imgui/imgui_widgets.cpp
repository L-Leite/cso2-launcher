                        ...
                        ImGuiComboFlags flags )
{
    ...
    else
    {
        ...
        IM_ASSERT( ImIsPowerOfTwo( flags &
                                   ImGuiComboFlags_HeightMask_ ) );  // Only one
                          ...
                          ImGuiInputTextFlags flags )
{
    ...
    for ( int i = 0; i < components; i++ )
    {
        PushID( i );
        value_changed |=
            InputScalar( "##v", data_type, v, step, step_fast, format, flags );
        SameLine( 0, g.Style.ItemInnerSpacing.x );
        PopID();
        PopItemWidth();
        v = (void*)( (char*)v + type_size );
                         ...
                         void* callback_user_data )
{
	...
	if ( is_multiline && ( flags & ImGuiInputTextFlags_NeedScrollDown ) )
    {
        SetWindowScrollY( draw_window, GetScrollMaxY() );
    }
                       ...
                       IM_ASSERT(
                            ...
                            (int)strlen(
                                callback_data.Buf ) );  // You need to maintain
                                                        // BufTextLen if you
                                                        // change the text! stuiped assert
             ...
             edit_state.StbState.select_end )
        {
            ...
            float bg_offy_up = is_multiline ?
                                   ...
                                   -1.0f;  // FIXME: those offsets should be
                                           // part of the style? they don't play
                          ...
                          ImGuiColorEditFlags flags, const float* ref_col )
{
    ...
    else if ( flags & ImGuiColorEditFlags_PickerHueBar )
    {
        ...
        sv_cursor_pos.y = ImClamp(
            (float)(int)( picker_pos.y + ImSaturate( 1 - V ) * sv_picker_size +
                          0.5f ),
...
void ImGui::SetColorEditOptions( ImGuiColorEditFlags flags )
{
    ...
    IM_ASSERT( ImIsPowerOfTwo(
        (int)( flags &
               ImGuiColorEditFlags__InputsMask ) ) );  // Check only 1 option is
                                                       // selected
    IM_ASSERT( ImIsPowerOfTwo(
        (int)( flags &
    ...
    IM_ASSERT( ImIsPowerOfTwo(
        (int)( flags &
               ImGuiColorEditFlags__PickerMask ) ) );  // Check only 1 option is
                                                       // selected
                           ...
                           ImGuiTabBarFlags flags )
{
    ...
    const float y = tab_bar->BarRect.Max.y - 1.0f;
    {
        const float separator_min_x =
            ...
            ( ( flags & ImGuiTabBarFlags_DockNodeIsDockSpace ) ?
                  0.0f :
...
static void ImGui::TabBarLayout( ImGuiTabBar* tab_bar )
{
    ...
    for ( int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++ )
    {
        ...
        // Refresh tab width immediately if we can (for manual tab bar,
        // WidthContent will lag by one frame which is mostly noticeable when
        // changing style.FramePadding.x) Additionally, when using
        // TabBarAddTab() to manipulate tab bar order we occasionally insert new
        // tabs that don't have a width yet, and we cannot wait for the next
        // BeginTabItem() call. We cannot compute this width within
                       ...
                       ImGuiTabItemFlags flags )
{
    ...
    if ( pressed || ( ( flags & ImGuiTabItemFlags_SetSelected ) &&
                      !tab_contents_visible ) )  // SetSelected can only be
...
void ImGui::SetTabItemClosed( const char* label )
{
    ...
    if ( is_within_manual_tab_bar )
    {
        ...
        IM_ASSERT(
            tab_bar
                ->WantLayout );  // Needs to be called AFTER BeginTabBar() and
                               ...
                               ImGuiTabItemFlags flags, ImU32 col )
{
    ...
    draw_list->AddConvexPolyFilled( draw_list->_Path.Data,
                                    draw_list->_Path.Size, col );
                                        ...
                                        ImGuiID close_button_id )
{
    ...
    if ( flags & ImGuiTabItemFlags_UnsavedDocument )
    {
        ...
        RenderTextClippedEx( draw_list, unsaved_marker_pos,
                             bb.Max - style.FramePadding, TAB_UNSAVED_MARKER,
                             NULL, NULL );
    }
    ...
    if ( label_size.x > text_ellipsis_clip_bb.GetWidth() )
    {
        ...
        {
            label_end = label + ImTextCountUtf8BytesFromChar(
        ...
        if ( !close_button_visible && ellipsis_x + ellipsis_width <= bb.Max.x )
            RenderPixelEllipsis(
                draw_list, ImVec2( ellipsis_x, text_pixel_clip_bb.Min.y ),
