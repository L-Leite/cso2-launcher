...
ImVec2 ImTriangleClosestPoint( const ImVec2& a, const ImVec2& b,
                               const ImVec2& c, const ImVec2& p )
{
...
// Pass data_size == 0 for zero-terminated strings, data_size > 0 for non-string
// data. Pay attention that data_size==0 will yield different results than
// passing strlen(data) because the zero-terminated codepath handles ###. This
// should technically be split into two distinct functions
// (ImHashData/ImHashStr), perhaps once we remove the silly static variable.
...
bool ImGuiListClipper::Step()
{
    ...
    if ( StepNo == 0 )  // Step 0: the clipper let you process the first
                        // element, regardless of it being visible or not, so we
                        // can measure the element height.
    {
        DisplayStart = 0;
        DisplayEnd = 1;
        StartPosY = ImGui::GetCursorPosY();
    ...
    else
    {
        ...
        if ( is_popup ||
             is_menu )  // Popups and menus bypass style.WindowMinSize by
                        ...
                        // facilitate understanding problematic cases (e.g.
                        // empty popups)
        ...
        else if ( held )
        {
            ...
            ImVec2 corner_target =
                ...
                ImLerp( grip.InnerDir * grip_hover_outer_size,
                        ...
                        grip.CornerPos );  // Corner of the window corresponding
                                           // to our corner grip
...
bool ImGui::Begin( const char* name, bool* p_open, ImGuiWindowFlags flags )
{
    ...
    if ( first_begin_of_the_frame )
    {
                ...
                ImVec2 size_for_clamping =
                    ( ( g.IO.ConfigWindowsMoveFromTitleBarOnly ) &&
                      !( window->Flags & ImGuiWindowFlags_NoTitleBar ) ) ?
                        ...
                        window->Size;
                window->Pos =
                    ImMax( window->Pos + size_for_clamping, padding ) -
                    size_for_clamping;
        ...
        if ( dim_bg_for_window_list && window == g.NavWindowingTargetAnim )
        {
            ...
            if ( !bb.Contains(
                     viewport_rect ) )  // Avoid drawing if the window covers
        ...
        else
        {
            ...
            if ( flags & ImGuiWindowFlags_MenuBar )
            {
                ...
                menu_bar_rect.ClipWith(
                    window
                        ->Rect() );  // Soft clipping, in particular child
        ...
        if ( !( flags & ImGuiWindowFlags_NoTitleBar ) )
        {
            ...
            if ( flags & ImGuiWindowFlags_UnsavedDocument )
            {
                ...
                ImVec2 off =
                    ImVec2( 0.0f, (float)(int)( -g.FontSize * 0.25f ) );
...
void ImGui::SetNextWindowFocus()
{
    ...
    g.NextWindowData.FocusCond =
        ImGuiCond_Always;  // Using a Cond member for consistency (may
                           // transition all of them to single flag set for fast
                           // Clear() op)
}
...
void ImGui::ClosePopupToLevel( int remaining, bool apply_focus_to_window_under )
{
    ...
    // It rarely manifested until now because UpdateMouseMovingWindow() would
    // call FocusWindow() again on the clicked window, leading to a chain of
    // focusing A (clicked window) then B (parent window of the popup) then A
...
bool ImGui::BeginPopupEx( ImGuiID id, ImGuiWindowFlags extra_flags )
{
    ...
    if ( !IsPopupOpen( id ) )
    {
        g.NextWindowData.Clear();  // We behave like Begin() and need to consume
                                   // those values
        return false;
    }
                ...
                if ( IsKeyPressed( g.IO.KeyMap[ImGuiKey_PageUp], true ) )
                {
                    ...
                    g.NavMoveDir =
                        ImGuiDir_Down;  // Because our scoring rect is offset,
                                        // we intentionally request the opposite
...
static void ImGui::NavUpdateWindowing()
{
    ...
    if ( apply_toggle_layer && g.NavWindow )
    {
        ...
        g.NavDisableHighlight = false;
        g.NavDisableMouseHover = true;
        NavRestoreLayer( ( g.NavWindow->DC.NavLayerActiveMask &
                           ( 1 << ImGuiNavLayer_Menu ) ) ?
    ...
    struct Funcs
    {
                ...
                while ( clipper.Step() )
                          ...
                          prim < clipper.DisplayEnd; prim++ )
                    {
                        ...
                        for ( int n = 0; n < 3; n++, vtx_i++ )
                        {
                            ImDrawVert& v =
                                draw_list
                                    ->VtxBuffer[idx_buffer ? idx_buffer[vtx_i] :
                                                             vtx_i];
        ...
        static void NodeWindow( ImGuiWindow* window, const char* label )
        {
            ...
            ImGui::BulletText( "Scroll: (%.2f/%.2f,%.2f/%.2f)",
                               window->Scroll.x, GetWindowScrollMaxX( window ),
                               window->Scroll.y,
        ...
        static void NodeTabBar( ImGuiTabBar* tab_bar )
        {
            ...
            // functionality) currently hold no discernable strings.
            char buf[256];
            char* p = buf;
