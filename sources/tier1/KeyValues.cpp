...
#define INTERNALWRITE( pData, len ) \
    InternalWrite( filesystem, f, pBuf, pData, len )
    ...
    void ReportError( const char* pError )
    {
        ...
        for ( int i = 0; i < m_maxErrorIndex; i++ )
        {
            if ( i < MAX_ERROR_STACK && m_errorStack[i] != INVALID_KEY_SYMBOL )
            {
                if ( i < m_errorIndex )
                {
                    Warning( "%s, ", KeyValues::CallGetStringForSymbol(
                                         m_errorStack[i] ) );
                }
                else
                {
...
public:
    ...
    int GetSymbolForString( const char* name, bool bCreate = true )
    {
        ...
        if ( bCreate )
        {
            ...
            if ( bInserted )
            {
                int iIndex = m_vecStrings.AddMultipleToTail(
                    V_strlen( name ) + 1, name );
                              ...
                              bool refreshCache )
{
    ...
    */
    ...
    bool bRetOK = ( ( (IFileSystem*)filesystem )
                        ->ReadEx( buffer, bufSize, fileSize, f ) != 0 );
    ...
    {
        KeyValuesSystem()->AddFileKeyValuesToCache( this, resourceName,
                                                    pathID );
    }
...
//-----------------------------------------------------------------------------
void KeyValues::RecursiveSaveToFile( IBaseFileSystem* filesystem,
                                     FileHandle_t f, CUtlBuffer* pBuf,
                               ...
                               bool bAllowEmptyString )
{
    ...
    else
    {
        ...
        switch ( dat->m_iDataType )
        {
            ...
            {
                if ( dat->m_sValue &&
                     ( bAllowEmptyString || *( dat->m_sValue ) ) )
                {
            ...
            case TYPE_WSTRING:
            {
                if ( dat->m_wsValue )
                {
                    ...
                    int result = Q_UnicodeToUTF8( dat->m_wsValue, buf,
                                                  KEYVALUES_TOKEN_SIZE );
                    if ( result )
                    {
                        WriteIndents( filesystem, f, pBuf, indentLevel + 1 );
                        INTERNALWRITE( "\"", 1 );
                        INTERNALWRITE( dat->GetName(),
                                       Q_strlen( dat->GetName() ) );
                        INTERNALWRITE( "\"\t\t\"", 4 );
            ...
            case TYPE_COLOR:
                DevMsg( 1, "KeyValues::RecursiveSaveToFile: TODO, missing code "
                           "for TYPE_COLOR.\n" );
                break;
                                  ...
                                  const char* defaultValue )
{
    ...
    if ( dat )
    {
        ...
        switch ( dat->m_iDataType )
        {
            ...
            case TYPE_WSTRING:
            {
                // convert the string to char *, set it for future use, and
                // return it
                char wideBuf[512];
                                ...
                                const char* pPathID )
{
    ...
    do
    {
        ...
        if ( !Q_stricmp(
                 s, "#include" ) )  // special include macro (not a key name)
        {
        ...
        if ( !pCurrentKey )
        {
            ...
            pCurrentKey->UsesEscapeSequences(
                m_bHasEscapeSequences != 0 );  // same format has parent use
                                         ...
                                         CUtlBuffer& buf )
{
    ...
    while ( 1 )
    {
        ...
        {
            bAccepted =
                !m_bEvaluateConditionals || EvaluateConditional( value );
        ...
        else
        {
            ...
            {
                g_KeyValuesErrorStack.ReportError(
                    "RecursiveLoadFromBuffer:  got conditional between key and "
                    "value" );
                break;
            }
    ...
    while ( pUnpackTable->m_pKeyName )
    {
        ...
        switch ( pUnpackTable->m_eDataType )
        {
            ...
            case UNPACK_TYPE_VECTOR:
            {
                ...
                char const* src_string = GetString(
                    pUnpackTable->m_pKeyName, pUnpackTable->m_pKeyDefault );
                if ( ( !src_string ) ||
                     ( sscanf( src_string, "%f %f %f", &( dest_v->x ),
                               &( dest_v->y ), &( dest_v->z ) ) != 3 ) )
                    dest_v->Init( 0, 0, 0 );
            }
            break;
            ...
            case UNPACK_TYPE_TWO_FLOATS:
            {
                ...
                char const* src_string = GetString(
                    pUnpackTable->m_pKeyName, pUnpackTable->m_pKeyDefault );
                if ( ( !src_string ) || ( sscanf( src_string, "%f %f", dest_f,
                                                  dest_f + 1 ) ) != 2 )
...
bool KeyValues::ProcessResolutionKeys( const char* pResString )
{
    ...
    for ( ; pSubKey != NULL; pSubKey = pSubKey->GetNextKey() )
    {
        ...
        if ( Q_stristr( pSubKey->GetName(), pResString ) != NULL )
        {
            ...
            V_strncpy( normalKeyName, pSubKey->GetName(),
                       sizeof( normalKeyName ) );
                                              ...
                                              int nIndentLevel )
{
    if ( pKey )
    {
        return KvWriteIndent( nIndentLevel ) &&
               KvWriteText( pKey->GetName() ) && KvWriteText( " {\n" );
    }
    else
    {
                                                ...
                                                int nIndentLevel )
{
    ...
    switch ( val->GetDataType() )
    {
        ...
        case KeyValues::TYPE_WSTRING:
        {
            ...
            V_snprintf( chBuffer, numBytes, "%ls [wstring, len = %d]", wsz,
                        nLen );
            if ( !KvWriteText( chBuffer ) )
                return false;
        }
