...
void RaiseLoadLibraryError( bool bFoundLibrary, const char* szModuleName )
{
    ...
    FormatMessageA( FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_ALLOCATE_BUFFER,
                    nullptr, dwLastError, NULL, (char*)&pMessage, NULL,
                    nullptr );
    V_snprintf( szMessageBuffer, sizeof( szMessageBuffer ),
                "IntLoadLib Failed(%d)\n%d:%s (%s)", bFoundLibrary, dwLastError,
                pMessage, szModuleName );
    LocalFree( pMessage );
...
static HMODULE InternalLoadLibrary( const char* pName, Sys_Flags flags )
{
    ...
    if ( !hResult )
    {
        if ( strcmp( pName, "shaderapiempty.dll" ) &&
             strcmp( pName, "shadercompile_dll.dll" ) )
...
{
    ThreadedLoadLibaryContext_t* pContext =
        (ThreadedLoadLibaryContext_t*)pParam;
    pContext->m_hLibrary =
        InternalLoadLibrary( pContext->m_pLibraryName, SYS_NOFLAGS );
    return 0;
}
...
HMODULE Sys_LoadLibrary( const char* pLibraryName, Sys_Flags flags )
{
    ...
    const char* pModuleExtension =
        pDllStringExtension ? ( pDllStringExtension - 1 ) : DLL_EXT_STRING;
                            ...
                            Sys_Flags flags /* = SYS_NOFLAGS (0) */ )
{
    ...
    if ( !Q_IsAbsolutePath( pModuleName ) )
    {
        ...
        Q_snprintf( szAbsoluteModuleName, sizeof( szAbsoluteModuleName ), "%s",
...
#if !defined( LINUX )
    // If running in the debugger, assume debug binaries are okay, otherwise
    // they must run with -allowdebug
    if ( Sys_GetProcAddress( hDLL, "BuiltDebug" ) )
    {
        if ( !IsX360() && hDLL && !CommandLine()->FindParm( "-allowdebug" ) &&
...
CreateInterfaceFn Sys_GetFactory( CSysModule* pModule )
{
    ...
    return ( CreateInterfaceFn )(
        GetProcAddress( (void*)hDLL, CREATEINTERFACE_PROCNAME ) );
#endif
}
...
CDllDemandLoader::CDllDemandLoader( char const* pchModuleName )
    : m_pchModuleName( pchModuleName ), m_hModule( 0 ),
...
typedef USHORT( WINAPI RtlCaptureStackBackTrace_FUNC )( ULONG frames_to_skip,
...
extern "C" int backtrace( void** buffer, int size )
{
    ...
    static RtlCaptureStackBackTrace_FUNC* const pfnRtlCaptureStackBackTrace =
        (RtlCaptureStackBackTrace_FUNC*)GetProcAddress(
